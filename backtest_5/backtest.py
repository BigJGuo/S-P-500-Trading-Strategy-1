...
# Behavior flags (overridable from the CLI):
#   USE_MIN_SHARE_FLOOR: if True, every triggered trim sells at least 1 share
#       (max(1, floor(shares * pct))). If False, the trim is skipped when
#       floor(shares * pct) rounds to 0 — restoring the original spec's
#       "no trade if size rounds to zero" behavior for small positions.
#   REINVEST_TO_SPY: if True, any cash accumulated from trims (and the small
#       leftover after the initial buy) is rolled into SPY at each day's
#       close so cash does not drag while the strategy waits.
USE_MIN_SHARE_FLOOR = True
REINVEST_TO_SPY     = False
...
def run_backtest(prices: pd.DataFrame):
    ...
    # SPY shares held by the STRATEGY (separate from the SPY benchmark above).
    # Only used when REINVEST_TO_SPY is True — accumulated as cash from trims
    # is rolled into SPY at each day's close.
    strategy_spy_shares = 0
    ...
    for i in range(n_days):
        ...
        if pending:
            for j in list(pending.keys()):
                ...
                # Percentage of CURRENT position value. target_usd / exec_price
                # == shares*pct since target_usd = shares*exec_price*pct.
                pct = PROFIT_SELL_PCT if trig_type == "profit_take" else STOP_SELL_PCT
                target_shares = int(math.floor(shares[j] * pct))
                if USE_MIN_SHARE_FLOOR:
                    target_shares = max(1, target_shares)
                sh_sell = min(int(shares[j]), target_shares)
                if sh_sell <= 0:
                    # Floor disabled and target rounded to 0 → skip without
                    # resetting reference, so it will keep re-triggering until
                    # the position is large enough for the % to fill ≥1 share.
                    pending.pop(j, None)
                    continue
        ...
        # ---- 4b2. (optional) roll free cash into SPY at today's close ----
        # When REINVEST_TO_SPY is on, any cash sitting idle gets converted to
        # SPY shares at the day's close. This eliminates the cash drag that
        # otherwise drowns a trim-heavy strategy in a bull market.
        if REINVEST_TO_SPY and cash > 1.0:
            sc_now = spy_close_series[i]
            if not np.isnan(sc_now) and sc_now > 0:
                spy_exec = sc_now * (1 + SLIPPAGE_PCT)
                per_share_cost = spy_exec * (1 + COMMISSION_PCT)
                sh = int(math.floor(cash / per_share_cost))
                if sh > 0:
                    cost = sh * per_share_cost
                    strategy_spy_shares += sh
                    cash -= cost
        ...
        if REINVEST_TO_SPY and strategy_spy_shares > 0:
            sc_now = spy_close_series[i]
            if not np.isnan(sc_now):
                held_value += strategy_spy_shares * sc_now
...
def main():
    ...
    global PROFIT_SELL_PCT, STOP_SELL_PCT, USE_MIN_SHARE_FLOOR, REINVEST_TO_SPY
    ...
    parser.add_argument("--pt-pct",  type=float, default=PROFIT_SELL_PCT,
                        help=f"Profit-take trim size (fraction of position). Default {PROFIT_SELL_PCT}.")
    parser.add_argument("--sl-pct",  type=float, default=STOP_SELL_PCT,
                        help=f"Stop-loss trim size (fraction of position). Default {STOP_SELL_PCT}.")
    parser.add_argument("--no-min-share", action="store_true",
                        help="Drop the max(1 share) floor: skip the trim if "
                             "floor(shares * pct) rounds to 0 instead of forcing 1.")
    parser.add_argument("--reinvest", action="store_true",
                        help="Roll trim cash into SPY at each day's close so "
                             "idle cash does not drag returns.")
    args = parser.parse_args()

    PROFIT_SELL_PCT     = args.pt_pct
    STOP_SELL_PCT       = args.sl_pct
    USE_MIN_SHARE_FLOOR = not args.no_min_share
    REINVEST_TO_SPY     = args.reinvest
    ...
    print(f"CONFIG  PT={PROFIT_SELL_PCT*100:.4g}%  SL={STOP_SELL_PCT*100:.4g}%  "
          f"min_share_floor={USE_MIN_SHARE_FLOOR}  reinvest_to_SPY={REINVEST_TO_SPY}")
