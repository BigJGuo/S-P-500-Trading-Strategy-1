...
Sales execute at the NEXT day's open. The dollar size of each partial sale
is scaled to starting capital: 0.01% on profit-takes ($10 per $100k) and
0.005% on stop-losses ($5 per $100k). After every actual partial sale the
reference price resets to the trigger-day close. When a remaining position
falls below $1 it is fully closed and never re-entered.
...
STOP_LOSS_PCT    = -0.05
PROFIT_SELL_FRAC = 0.0001    # 0.01%  of starting capital per profit-take
STOP_SELL_FRAC   = 0.00005   # 0.005% of starting capital per stop-loss
...
PROFIT_SELL_USD = STARTING_CAPITAL * PROFIT_SELL_FRAC
STOP_SELL_USD   = STARTING_CAPITAL * STOP_SELL_FRAC
...
def load_prices(tickers: list[str], refresh: bool = False) -> pd.DataFrame:
    ...
    if CACHE_PATH.exists() and not refresh:
        ...
        df.columns = pd.MultiIndex.from_tuples(
            [tuple(c.split("|")) for c in df.columns], names=["Ticker", "Field"]
        )
    ...
    for i in range(n_days):
        ...
        if pending:
            for j in list(pending.keys()):
                ...
                if trig_type == "profit_take":
                    target_usd = PROFIT_SELL_USD
                else:  # stop_loss
                    target_usd = STOP_SELL_USD

                sh_sell = min(int(shares[j]), int(math.floor(target_usd / exec_price)))
                if sh_sell <= 0:
                    # target too small for one share at this price -> skip, do not reset
                    pending.pop(j, None)
                    continue
...
def main():
    ...
    global CACHE_PATH, TRADE_LOG, METRICS_TXT, METRICS_CSV, EQUITY_PNG, DRAWDOWN_PNG
